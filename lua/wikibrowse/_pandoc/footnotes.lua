...
function Superscript(elem)
        ...
        for _, inline in ipairs(elem.content) do
                if
                        inline.t == "Link"
                        and inline.target:find("#cite_note", 1, true)
                then
...
function Div(elem)
        if has_class(elem, "reflist") then
                ...
                for i, item_blocks in ipairs(ol.content) do
                        ...
                        if
                                first_block.t == "Plain"
                                and first_block.content[1].attr.identifier:find(
                                        "cite_note",
                                        1,
                                        true
                                )
                        ...
                        if #content_blocks > 0 then
                                local identifier =
                                        first_block.content[1].attr.identifier
                                local note_label = references[identifier]
                                if not note_label then
                                        local num = identifier:match(
                                                "^cite_note%-(%d+)$"
                                        )
                                        if num then
                                                note_label = num
                                ...
                                local note_key = pandoc.RawInline(
                                        "markdown",
                                        "[^" .. note_label .. "]:"
                                )
                                local first_content_block = content_blocks[1]
                                if
                                        first_content_block.t == "Para"
                                        or first_content_block.t
                                                == "Plain"
                                then
                                        table.insert(
